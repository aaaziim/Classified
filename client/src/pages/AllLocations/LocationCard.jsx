      ...
      <div className="space-y-4">
        <div className="bg-white rounded-lg p-4 shadow-md ">
          ...
          <Link to={`/location/${_id}`}>
                  <button
                    className="w-full text-center font-medium text-[#014D48] text-lg cursor-pointer"
                    title={name}
                  >
                    {name}
                  </button>
              ...
              <div className="collapse-content text-sm">
                <ul className=" grid  grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2 mt-3 space-y-2 pl-5 text-center">
                  {location?.state?.map((sub, index) => (
                    <li
                      key={sub._id}
                      className="block text-[#FA8649] text-sm hover:text-[#001C27] transition duration-300"
                    >
                      <Link to={`/state/${sub.name}`}>{sub.name}</Link>
                    </li>
                  ))}
                </ul>
              </div>
